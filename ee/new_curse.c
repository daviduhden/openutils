...
static char *TERMINAL_TYPE = NULL; /* terminal type from the environment */
static int Data_Line_len = 0;
static int Max_Key_len;	/* max length of a sequence sent by a key */
static char *Data_Line = NULL;
static const char *TERM_PATH = NULL;
static char *TERM_data_ptr = NULL;
static char *Term_File_name = NULL;	/* file containing terminal description */
static int Fildes;	/* file descriptor for the terminfo file */
static int Time_Out;	/* set when time elapsed while reading a function key */
static int Curr_x;	/* current x position on screen */
static int Curr_y;	/* current y position on the screen */
int LINES;
int COLS;
static int initialized = FALSE;	/* tells whether new_curse is initialized */
static int Repaint_screen;	/* repaint screen if an operation is impossible */
static int Intr;	/* storage for the interrupt character */
static int Parity;	/* 0 = no parity, 1 = odd parity, 2 = even parity */
static int Noblock;	/* non-blocking reads in progress */
static int Num_bits;	/* number of bits per character */
static int Flip_Bytes;	/* terminfo file needs byte swapping */
static float chars_per_millisecond;	/* terminfo padding delays */
static int STAND = FALSE;	/* standout mode activated */
static volatile sig_atomic_t nc_resize_flag;	/* set on SIGWINCH */

static char *Strings;

static struct nc_line *top_of_win;
...
void copy_window(WINDOW *origin, WINDOW *destination)
{
	...
	destination->LX = nc_min((destination->Num_cols - 1), origin->LX);
	destination->LY = nc_min((destination->Num_lines - 1), origin->LY);
...
/*
 *	SIGWINCH handler: async-signal-safe; the actual resize is
 *	performed by wgetch() in normal context.
 */
static void
...
void initscr(void)	/* initialize terminal for operations */
{
...
#endif
	/*
	 * A hostile or malformed terminal description can carry zero or
	 * negative dimensions, which the line-list code below cannot
	 * represent; clamp to sane defaults like other curses
	 * implementations do.
	 */
	if (LINES < 1)
		LINES = 24;
	...
	/*
	 ...
	 |	the resize is performed in normal context by wgetch().
	 |	SA_RESTART is deliberately not set so that a blocked
	 |	read(2)/select(2) is interrupted and the flag is
	 |	processed promptly.
	 */
	{
		struct sigaction sa;
...
keys_vt100(void)
{
	int counter;
	int Klen;
	struct KEY_STACK *Spoint, *last;

	/*
	 * Append the built-in vt100 key definitions to the key list.
	 * The list may be empty (a terminal entry without any key
	 * capabilities); the old code dereferenced the NULL head here.
	 */
	last = NULL;
	...
	for (counter = 0; vt100[counter].length != 0; counter++)
	{
		...
		if (n == NULL)
			break;
		n->next = NULL;
		n->element = &vt100[counter];
		if (last == NULL)
			KEY_TOS = n;
		else
			last->next = n;
		last = n;
		Klen = strlen(n->element->string);
		if (Klen > Max_Key_len)
			Max_Key_len = Klen;
	}
}
...
static float chars;
static int p[10];
static int variable[27];

/*
 *	Terminfo conditional operations, evaluated on a small stack.
 *	A malformed TERMINFO entry must not be able to drive the stack
 *	out of bounds, so all accesses go through bounded helpers.
 */
static int
nc_pop(int *stack, int *place)
{
	if (*place <= 0 || *place > 128)
		return (0);
	return (stack[--(*place)]);
}

static void
nc_push(int *stack, int *place, int value)
{
	if (*place >= 128)
		return;
	stack[(*place)++] = value;
...
Operation(int *Temp_Stack, int place)	/* handle conditional operations	*/
{
	...
	else if (*Otemp == '/')
	{
		...
		temp = nc_pop(Temp_Stack, &place);
		{
			int divisor = nc_pop(Temp_Stack, &place);
...
/*
 *	Wide-character input used by ee.  Single-byte characters are
 *	returned as is; escape sequences decoded by Get_key() yield
 *	KEY_* codes returned with KEY_CODE_YES; UTF-8 sequences are
 *	assembled into a single wide character.  Invalid or incomplete
 *	sequences fall back to returning the first byte, with any
 *	consumed continuation bytes pushed back.
 */
int
wget_wch(WINDOW *window, wint_t *wch)
{
	int c;
	...
	if (need > 1) {
		...
		while (len < need) {
			...
			if (nxt == -1)
				break;
			buf[len++] = (unsigned char)nxt;
		}
	}
	memset(&mbs, 0, sizeof(mbs));
	got = mbrtowc((wchar_t *)wch, (const char *)buf, len, &mbs);
	if (got == (size_t)-1 || got == (size_t)-2 || got != len) {
		while (len > 1)
			ungetch(buf[--len]);
		*wch = (wint_t)c;
	}
...
void
overwrite(WINDOW *origin, WINDOW *destination)
{
...
Get_key(int first_char)	/* try to decode key sequence */
{
	...
	struct KEY_STACK *St_point = NULL;
...
nl(void)
{
	Terminal.c_iflag |= ICRNL;	/* map carriage-return to line-feed */
	(void)_nc_tset(&Terminal);
}
...
nonl(void)
{
	Terminal.c_iflag &= ~ICRNL;	/* no carriage-return to line-feed mapping */
